...
const zones = [
  { title: "대명 문화예술권", description: "공연거리 · 계대네거리 · 명덕역", className: "border-[#FFE88A] bg-[#FFF7D6]" },
  { title: "봉덕·이천 생활권", description: "앞산 생활복지 · 보행안전", className: "border-[#B9DBFF] bg-[#EAF4FF]" },
  { title: "앞산·안지랑 관광권", description: "체류형 관광 · 가족문화", className: "border-[#D0D5DD] bg-[#F8FAFC]" },
  { title: "후적지·교통축", description: "캠프조지 · 서부정류장 · 순환도로", className: "border-[#E5E7EB] bg-white" },
];
...
export function StaticMap({ preview = false }: { preview?: boolean }) {
  ...
  return (
    <div className={`grid gap-6 ${preview ? "" : "lg:grid-cols-[1.08fr_0.92fr]"}`}>
      {!preview ? (
        <div className="lg:col-span-2 grid gap-3 md:grid-cols-4">
          {zones.map((zone) => (
            <article key={zone.title} className={`rounded-[1.4rem] border p-4 shadow-sm ${zone.className}`}>
              <p className="font-black text-[#11205A]">{zone.title}</p>
      ...
      <div className="rounded-[2rem] border border-[#B9DBFF] bg-[#EAF4FF] p-3 shadow-xl shadow-blue-900/10">
        <div className="overflow-hidden rounded-[1.5rem] bg-white">
          ...
          <div className="relative h-[540px] overflow-hidden bg-[#F8FBFF] md:h-[640px]">
            <svg className="absolute inset-0 h-full w-full" viewBox="0 0 100 100" role="img" aria-label="대구 남구 공약지도">
              ...
              <path d="M14 20 L58 12 L84 34 L78 84 L42 92 L16 70 Z" fill="#EAF4FF" stroke="#B9DBFF" strokeWidth="1.2" />
              ...
              <path d="M19 48 C32 43 47 47 58 50 C65 52 72 50 80 46" fill="none" stroke="#0052B8" strokeWidth="1.3" strokeDasharray="2 2" opacity=".45" />
              <path d="M42 91 C48 75 53 62 58 50 C62 40 68 35 82 34" fill="none" stroke="#0052B8" strokeWidth="1.3" strokeDasharray="2 2" opacity=".45" />
            </svg>
            {markers.map((marker) => {
              ...
              return (
                <button key={marker.title} type="button" onClick={() => setSelected(marker)} className="absolute -translate-x-1/2 -translate-y-full" style={{ left: `${marker.x}%`, top: `${marker.y}%` }} aria-label={`${marker.title} 공약 보기`}>
                  <span className={`flex h-10 w-10 items-center justify-center rounded-full border-4 border-white shadow-xl transition ${active ? "scale-110 bg-[#0052B8] text-white ring-4 ring-[#FFD84D]/70" : categoryStyles[marker.category].marker}`}><MapPin size={19} fill="currentColor" aria-hidden /></span>
                </button>
              );
            })}
            ...
            <div className="absolute left-4 right-4 top-4 grid gap-2 sm:grid-cols-2 md:right-auto md:w-[360px]">
              {zones.map((zone) => (
                <div key={zone.title} className="rounded-2xl border border-white/80 bg-white/90 px-3 py-2 shadow-sm backdrop-blur">
                  <p className="text-xs font-black text-[#11205A]">{zone.title}</p>
