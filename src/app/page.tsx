...
export default function HomePage() {
  return (
    <>
      ...
      <section className="py-16"><div className="container-page grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center"><div><SectionTitle eyebrow="1호 공약" title="대명 프로젝트" description="남구는 자생적 예술촌의 힘을 가진 도시입니다. 행정이 보지 못했던 현장의 가치를 예술가와 주민이 함께 살리겠습니다." /><div className="mt-6 flex flex-wrap gap-2">{["대명공연거리", "지역쿼터제", "공연비 선지급제", "다원공연예술", "중간지원조직"].map((item) => <span key={item} className="rounded-full bg-[#EAF4FF] px-4 py-2 text-sm font-black text-[#0052B8]">{item}</span>)}</div><Link href="/daemyeong" className="mt-8 inline-flex items-center gap-2 rounded-full bg-[#0052B8] px-6 py-4 font-black text-white">대명 프로젝트 자세히 보기<ArrowRight size={18} /></Link></div><div className="rounded-[2rem] bg-[#11205A] p-8 text-white"><Building2 size={42} className="text-[#FFD84D]" /><p className="mt-6 text-3xl font-black leading-tight">남구를 대구 문화예술의 심장으로</p><p className="mt-4 leading-8 text-blue-50">대명공연거리, 계대네거리, 명덕역을 잇는 문화예술축을 만들겠습니다.</p></div></div></section>
      <section className="bg-[#11205A] py-16 text-white"><div className="container-page grid gap-8 lg:grid-cols-[1fr_1fr] lg:items-center"><div><p className="inline-flex rounded-full bg-[#FFD84D] px-4 py-2 text-sm font-black text-[#11205A]">현장형 후보</p><h2 className="mt-5 text-4xl font-black md:text-5xl">거리에서 듣고, 구청에서 바꾸겠습니다</h2><p className="mt-5 text-lg leading-8 text-blue-50">남구청장 후보 정연우는 책상 위 보고서보다 거리의 목소리를 먼저 듣겠습니다. 주민의 불편을 행정의 언어로 바꾸겠습니다.</p><Link href="/voice" className="mt-7 inline-flex rounded-full bg-white px-6 py-4 font-black text-[#11205A]">의견 남기기</Link></div><CampaignImageBlock src={siteConfig.images.fieldCampaign} alt="거리에서 시민을 만나는 정연우 남구청장 후보 배너" title="거리에서 듣겠습니다" description="현장에서 시작하는 남구 행정" /></div></section>
      <section className="py-16"><div className="container-page"><SectionTitle eyebrow="남구 공약지도" title="남구의 핵심 현안을 한눈에" description="대명공연거리, 앞산, 안지랑, 후적지와 생활교통 공약을 지도형으로 확인하세요." /><div className="mt-8"><StaticMap preview /></div><Link href="/map" className="mt-8 inline-flex items-center gap-2 rounded-full bg-[#0052B8] px-6 py-4 font-black text-white">남구 공약지도 보기<ArrowRight size={18} /></Link></div></section>
